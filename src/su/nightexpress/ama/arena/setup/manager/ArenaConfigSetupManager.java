...
public class ArenaConfigSetupManager extends AbstractSetupManager<IArenaConfig> {
    ...
    @Override
    protected void onSetupStart(@NotNull Player player, @NotNull IArenaConfig arenaConfig) {
    ...
    @Override
    protected void onSetupEnd(@NotNull Player player, @NotNull IArenaConfig arenaConfig) {
        arenaConfig.getEditor().open(player, 1);
    }

    @Override
    protected void handleItem(@NotNull PlayerInteractEvent e, @NotNull Player player, @NotNull IArenaConfig arenaConfig, @NotNull ItemStack item, @NotNull SetupItemType itemType) {
        switch (itemType) {
            case ARENA_LOCATION_LOBBY -> {
                ...
                plugin.lang().Setup_Arena_Lobby_Set.replace(arenaConfig.replacePlaceholders()).send(player);
            }
            case ARENA_LOCATION_LEAVE -> {
                ...
                if (action == Action.RIGHT_CLICK_BLOCK || action == Action.RIGHT_CLICK_AIR) {
                    plugin.lang().Setup_Arena_Leave_Set.replace(arenaConfig.replacePlaceholders()).send(player);
                    arenaConfig.setLocation(ArenaLocationType.LEAVE, location);
                }
                else if (action == Action.LEFT_CLICK_BLOCK || action == Action.LEFT_CLICK_AIR) {
                    plugin.lang().Setup_Arena_Leave_UnSet.replace(arenaConfig.replacePlaceholders()).send(player);
            ...
            case ARENA_LOCATION_SPECTATE -> {
                ...
                plugin.lang().Setup_Arena_Spectate_Set.replace(arenaConfig.replacePlaceholders()).send(player);
            }
